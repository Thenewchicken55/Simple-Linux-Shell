...
struct command_t translateCommand(struct command_t *cmd) {
    ...
    if (strcmp(cmd->name, "C") == 0) {
        ...
        printf("One funny use-case is 'X make run' or 'X ./myshell' -- this will run the shell program in the shell program, this can be done endlessly until your system runs out of threads \n");
        printf("\nAdditional Notes\n");
